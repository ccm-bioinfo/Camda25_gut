...
def main():
    ...
    conditions = dict()
    conditions['GI'] = ['CRC', 'adenoma', 'IBD', 'UC', 'CD', 'acute_diarrhoea', 'few_polyps']
    conditions['MC'] = ["T2D","IGT","metabolic_syndrome","hypercholesterolemia","adenoma;hypercholesterolemia","T2D:adenoma","CRC:hypercholesterolemia"]
    conditions['CV'] = ["ACVD", "CAD","HF","CAD:T2D","HF:CAD","HF:CAD:T2D"]
    conditions['PC'] = [ "PD","BD","schizophrenia", "IGT:MS", "ME/CFS"]
    conditions['ALL'] = list(set(metadata[args.diagnosis]))
    ...
    # print(metadata)
        ...
        fold_results.to_csv(f'{condition}_f1_score_results_pathways.csv')

        results_final2 = pd.concat(results_final, ignore_index=True)
        print(results_final2)
        results_final2.to_csv(f'{condition}_camda2025_preds_pathways.csv', index = False)
