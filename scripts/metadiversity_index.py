...
#writing on a CSV:
field_names = ['SAMPLE','CONTIGS','RICHNESS','SHANNON','SIMPSON','LOG10_MD','MDI']
...
# Total Contigs
O = len(clustlist)
#print('Total protein-encoding genes: ', O, file = f)


# Protein Richness
P = len(clustdist)
#print('Protein Richness: ', P, file = f)
...
log10_prote = np.log10(np.sum(MD))
MDindex = (1/O)*np.sum(MD)

#print('Log10 Protein Dissimilarity:', np.log10(np.sum(MD)))
#print('Metagenomic Diversity index:', (1/O)*np.sum(MD))

#Writing results on a CSV 
with open('MD_results.csv', 'a', newline= '') as f:
	write = csv.writer(f)
	write.writerow([sample, O, P, shannon, simpson, log10_prote, MDindex])
	f.close()
